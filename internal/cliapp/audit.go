...
// AuditDir implements the audit-dir CLI: aggregate AI-provenance audit over a
// directory tree — recursively inspects supported text/image/container files
// and emits one summary plus a per-file finding list with confidence
// classifications.
...
// AuditDir implements the audit-dir CLI.
func AuditDir(args []string) int {
	fs := newFlagSet("audit-dir")
	var jsonOut bool
	var skip string
	fs.BoolVar(&jsonOut, "json", false, "Emit a JSON report")
	fs.StringVar(&skip, "skip", "", "Comma-separated extra directory names to skip")
	var langFlag string
...
// AuditWebsite implements the audit-website CLI: aggregate AI-provenance audit
// over the URLs listed in a sitemap — downloads each URL, classifies it by
// content type/suffix/magic, and runs the same deterministic
// text/image/container inspections used by the local audit. Optional external
// tools (c2patool/exiftool) are not invoked for remote URLs; download the
// assets and run audit-dir locally for those.
//
...
func AuditWebsite(args []string) int {
	const (
	...
	_ = maxSitemapDecompressedBytes
	_ = maxRedirects
	_ = userAgent

	extForKind := map[string]string{
	...
	parseOrigin := func(u *url.URL) (urlOrigin, error) {
	...
	urlOriginOf := func(rawURL string) (urlOrigin, error) {
	...
	originAllowed := func(candidate, expected urlOrigin) bool {
	...
	isGlobalIP := func(ip net.IP) bool {
		if ip.To4() != nil {
			ip = ip.To4()
		}
	...
	resolvePublicAddresses := func(o urlOrigin) ([]string, error) {
	...
	validatedTarget := func(rawURL string, expectedOrigin *urlOrigin) (urlOrigin, []string, error) {
	...
	readHTTPResponse := func(conn net.Conn) (*http.Response, error) {
		req, _ := http.NewRequest("GET", "/", nil)
		return http.ReadResponse(bufio.NewReader(conn), req)
	}

	requestTarget := func(rawURL string) string {
		u, err := url.Parse(rawURL)
		if err != nil {
			return "/"
		}
	...
	resolveRef := func(base, ref string) (string, error) {
	...
	readCapped := func(r io.Reader, maxBytes int) ([]byte, error) {
	...
	fetch := func(rawURL string, timeout, maxBytes int, allowedOrigin *urlOrigin) ([]byte, string, error) {
		currentURL := rawURL
	...
	guessKind := func(rawURL string, data []byte, contentType string) string {
		...
		if len(head) > 0 && head[0] == '<' && bytes.Contains(bytes.ToLower(firstN(data, 500)), []byte("svg")) {
			return "svg"
		}
		if bytes.Contains(bytes.ToLower(firstN(data, 2000)), []byte("<html")) || (len(head) > 0 && head[0] == '<') {
			return "html"
		}
		return "text"
	}

	localName := func(tag string) string {
	...
	parseSitemap := func(data []byte) (string, []string, error) {
	...
	inspectRemote := func(rawURL string, data []byte, contentType string) (map[string]interface{}, error) {
	...
	discoverSitemap := func(baseURL string, timeout int) (string, error) {
	...
	collectURLs := func(sitemapURL string, timeout, maxPages int) ([]string, error) {
		var urls []string
	...
	var jsonOut bool
	fs.StringVar(&sitemap, "sitemap", "", "Sitemap URL to audit")
	fs.StringVar(&baseURL, "base", "", "Base URL; discover the sitemap automatically")
	fs.IntVar(&maxPages, "max-pages", defaultMaxPages, "maximum pages to audit")
	fs.IntVar(&timeoutSec, "timeout", defaultTimeout, "per-request timeout in seconds")
	fs.IntVar(&maxBytes, "max-bytes", defaultMaxBytes, "per-asset byte cap")
	fs.BoolVar(&jsonOut, "json", false, "emit a JSON report")
	var langFlag string
	cliutil.AddLangFlagFS(fs, &langFlag)
