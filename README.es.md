...
Detecta y elimina marcas de procedencia de IA en textos, imágenes, documentos y
vídeos — esteganografía Unicode invisible, metadatos de imagen C2PA/EXIF/XMP,
metadatos de contenedores (PDF, DOCX, ODT, SVG, HTML, Markdown, vídeo
...
- **Vídeo** — best-effort: escaneo de cajas C2PA uuid/JUMBF, átomo QuickTime
  `©too`, escaneo de marcadores, borrado con `exiftool -all=`
...
./bin/watermarks-server # HTTP + web en 127.0.0.1:8765
```
...
# configurar: sudoedit /etc/watermarks-remover.env   (puerto, clave API, auto-limpieza…)
...
| --- | --- | --- |
| `WATERMARKS_SERVER_HOST` | `127.0.0.1` | dirección de enlace (solo loopback salvo proxy inverso) |
| `WATERMARKS_SERVER_PORT` | `8765` | puerto |
| `WATERMARKS_SERVER_API_KEY` | vacío | exige `Authorization: Bearer <clave>` si se define |
| `WATERMARKS_LANG` | idioma del sistema | `en` `zh` `es` `fr` `ru` |
| `WATERMARKS_AUTO_CLEAN` | `0` | `1` activa el conserje en segundo plano |
| `WATERMARKS_AUTO_CLEAN_INTERVAL` | `15m` | periodo de comprobación |
| `WATERMARKS_AUTO_CLEAN_THRESHOLD` | `11` | % libre que dispara la limpieza |
| `WATERMARKS_AUTO_CLEAN_TTL` | `24h` | retención de descargas antes de expulsarlas |
...
```bash
claude mcp add watermarks-remover -- /ruta/abs/a/bin/watermarks-mcp
# mcp.json de Cursor / Windsurf / Cline:
{ "mcpServers": { "watermarks-remover": { "command": "/ruta/abs/a/watermarks-mcp" } } }
```
